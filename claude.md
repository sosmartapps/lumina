...
## Active Issues
- **Vehicles + Social Media (2026-07-13)** built but NOT device-tested — User Profile has new "Vehicles" (multi, w/ photos) and "Social Media" tabs; data stored on `user_profile/profile` doc, vehicle photos → Storage `vehicle_photos/{patientId}/`. Wired into the EXISTING generators (no new flyer): QuadTrack "Share Missing Person Alert" (preview card + shared package now show vehicle photos + social handles via `allVehicles`) and the text Lost Person Report. **REDEPLOY** `firebase deploy --only storage` (new vehicle_photos rule). Test: add/edit/delete vehicle + photo capture, social link opens browser, then QuadTrack Share shows vehicles+social (see CHANGELOG)
...
- Home environment monitoring (2026-07-13) built but NOT device-tested — 3 paths behind `environment_connections/{patientId}`: SensorPush cloud API (needs G1 gateway), Nest SDM, and BLE bridge (patient phone reads HT.w directly via flutter_blue_plus, port of So Smart Scans GATT; `sensorpush_ble_bridge.dart`, starts at splash in USER mode only). Dashboard card, link/toggle/threshold screen, `pollEnvironment` fn + `onEnvironmentReading` trigger (BLE alerts). Rules updated (patient can write own env doc) — REDEPLOY rules. Nest needs `NEST_FN_CLIENT_ID/SECRET` secrets + `NEST_*` .env (Device Access reg, $5). Test BLE first w/ Leon's HT.w: rebuild (new plugin), caregiver toggles Bluetooth sensor on, patient-mode phone near sensor, watch `EnvBLE:` logs
