...
## Active Issues
- **Vehicles + Social Media + Missing-Person Flyer PDF (2026-07-13)** built but NOT device-tested — User Profile has new "Vehicles" and "Social Media" tabs; new `MissingPersonFlyerService` PDF flyer (button under the Lost Person Report banner). Vehicle photos → Storage `vehicle_photos/{patientId}/`; vehicles/social stored on `user_profile/profile` doc. **REDEPLOY** `firebase deploy --only storage` (new vehicle_photos rule). Test: add/edit/delete vehicle + photo capture, social link opens browser, flyer generate→photo embeds→print/share (see CHANGELOG)
