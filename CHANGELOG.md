...
## 2026-07-13 (pm) - Vehicles + Social Media + Missing-Person Flyer (PDF) BUILT — needs device test
...
- **Missing-Person Flyer (PDF)**: new `MissingPersonFlyerService.generateFlyer` — red MISSING banner, most-recent patient photo (auto), name/preferred, physical description grid, Last Seen panel (per-incident dialog: date/time, location, wearing, contact, notes), Areas to Check, Medical Alert, Vehicle blocks (with photos fetched via http → `pw.MemoryImage`), Social handles, dedup'd "CALL 911 + contact" footer. Opened via `PdfPreview` (print/share built in). Button added under the existing Lost Person Report banner. Text report (`formatLostPersonReportAsText`) also gained VEHICLE(S) + SOCIAL MEDIA sections.
...
- Verified: PDF layout rendered to PNG (both pages) and inspected; unit tests for full + empty profile in `test/missing_person_flyer_test.dart`. NOT device-tested — needs: add/edit/delete vehicle + photo capture on device, social link open-in-browser, flyer generate → photo embeds → print/share. **REDEPLOY**: `firebase deploy --only storage`.
